...
def train(T,
    ...
    for idx in range(max_updates):
        ...
        if method=="bsde":
            loss, _, _ = fbsde.bsdeint(ts=ts, x0=x0, option=asian, lag=lag)
        else:
            loss, _, _ = fbsde.conditional_expectation(ts=ts, x0=x0, option=asian, lag=lag)
        loss.backward()
        optimizer.step()
        ...
        if idx%10 == 0:
            with torch.no_grad():
                ...
                loss, Y, payoff = fbsde.bsdeint(ts=ts,x0=x0,option=asian,lag=lag)
